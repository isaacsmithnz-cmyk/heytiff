...
- **A unit on the plan is a solid block** in its system's colour with a white knockout around it and its louvres or fan knocked out of it in white. It was a 14% tint of that colour sitting on a zone filled with the same colour at 13%, so a head read as a scratch and, zoomed out, as nothing: the outline that carried it is non-scaling and never thickens. Solid, every system colour clears the 3:1 graphic floor against its own zone (the lightest, teal, at 3.2). The printed plan is NOT this — paper keeps a white body and a coloured edge.
