...
- **A unit on the plan is a white object outlined in its system's colour.** It was a 14% tint of that colour sitting on a zone filled with the same colour at 13%, so a head read as a scratch and, zoomed out, as nothing. The separation is the KNOCKOUT, not the luminance: white on a 13% tint is 1.15:1 and never more, but the body is opaque, so it covers the zone's tint and the grid dots under it and a real edge appears where the drawing stops. The coloured line is the one thing carrying contrast, so it is set one stop toward the ink — raw, the lightest system colour reads 2.7:1 against its own zone and misses the 3:1 graphic floor; darkened it clears at 3.3, and the darkest at 4.8. Screen and paper now draw a unit the same way.
