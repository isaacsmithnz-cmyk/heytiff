...
- **Every head that blows into the room carries an arrow leaving its discharge face,** in the air colour, with the same head the ducted flow arrow uses, so air is drawn one way on this plan. The pattern is the form factor's — one throw out the front for a wall head, floor unit or under-ceiling, four for a 4-way cassette, one for a 1-way — and it turns with the unit. The arrow is screen-sized like a label, never scaled to the footprint: a wall head is ten pixels deep at working zoom and anything proportional to that is a smudge. Ducted and bulkhead units blow through ducts, which their plenums draw; an outdoor unit gets no arrow.
