...
- **The open card's last slot is the next step:** Build system, then the units to place, then Answer install questions. Edit system sits in the header once a unit is in; the Studio already says Edit room. Add zones ends the zone chips.
...
- **They ask only what the plan and the pack can't answer.** The house's answers, its walls, are asked once and kept for every system. The plan answers where the outdoor sits. The pack sizes the isolator and adds the joint pipes a port needs.
...
- **Install questions.** The cockpit's Components tab becomes the questions: its choices, mounting, isolator and lagging, become answers, the house's answers are kept on the design, and the pack's accessories come in where the pack has them. The catalogue's fixings wait on your list of what you stock for each wall.
