...
- **The equipment list is a list, read down,** so it is set in three weights and two sizes and nothing else: a group is a heading in ink with a rule under it; a line is its name in ink with its quantity in ink beside it; and everything that qualifies a line — the model, why it is there, that it is for the day — is the quiet size under or beside the thing it qualifies, inside the line, never a row of its own. A line still waiting on an answer is quiet all the way across, so it is not mistaken for a part.
