...
- **A card's identity sits down the left:** the name with what it is beside it on the one line, and whose it is under them — which leaves the top right for the cross. The header's button says Add; the title beside it already says what.
...
- **The rotate handle is a badge off the footprint's top-right corner** — a white disc with the rotate cursor's curved arrow on it, in the system's colour, 18px across at any zoom, turned with the unit so the grab target tracks the drawn handle. No leg: nothing crosses the unit's face. It was a leg out of the top face with a knob on the end, and the leg was sized to the grid — 71px long at 140%, gone at fit-to-screen — the one thing on a selected unit that was not sized to the screen, and it looked like it. Drag the badge to spin; Shift snaps 15°; [ and ] step 90°.
