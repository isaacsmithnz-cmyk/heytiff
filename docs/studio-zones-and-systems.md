...
- **One question at a time, Back and Next.** The whole run in front of you was a form, and a form gets read before it gets answered. The step says which group it belongs to and what the pack knows about it, then asks. Next is never held back — an unanswered question is a waiting row, never a block — and the last step's Next is Done. Re-opening a part-answered system starts where the answers stop.
- **An answer can ask a follow-up:** a wall asks which bracket, the ground asks what it stands on, and each follow-up takes more than one answer too. A follow-up is a step of its own, straight after the answer that opened it, and it says which answer that was, so it belongs to that question without being tucked under it.
- **The equipment list stands beside the step the whole way.** Asking one at a time is only worth it if you can watch the list answer back.
