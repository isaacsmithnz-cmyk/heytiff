...
- **The equipment list is a list, read down,** and it climbs the same ladder the questions beside it climb — a title at 20, a group at 16, a line at 14, its qualifiers at 12 — because the two columns are read together and nobody should learn a second scale for the right-hand one. A group is a heading in ink; a line is its name in ink with its quantity in ink beside it; and everything that qualifies a line — the model, why it is there, that it is for the day — is the quiet size under or beside the thing it qualifies, inside the line, never a row of its own. A line still waiting on an answer is quiet all the way across, so it is not mistaken for a part.
