...
- **Every head the pack knows carries a solid arrow on its body,** in the air colour, pointing out of the discharge face: a stem the depth of the body and a filled head. One for a wall head, floor unit, under-ceiling, ducted box or 1-way cassette; four for a 4-way, each starting off the centre so they read as four and not a cross. It turns with the unit, and it rides the placing ghost, so which way a unit will face is known before it is let go of. Sized to the screen where it counts — never shorter than 16px, a 5px head — because a wall head is ten pixels deep at working zoom and anything proportional to that is a smudge. An air-capable ducted unit hands over to its own flow arrow once a plenum has oriented it. An outdoor unit gets nothing.
...
- **The card's last row is Edit system and Delete, two thirds to one third.** A system can go without opening its builder; the way you take stands in two thirds of the row and the way you cannot undo takes the other third. Delete is labelled, never a cross — a chip's cross clears one zone, and this takes a whole system and its units off the plan — and the word sits quiet with Edit until it is pointed at, when it turns red. Its zones go back to unclaimed and it costs one undo, the same as the builder's.
