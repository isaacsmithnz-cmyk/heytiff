...
the diary no longer reads `is_debrief`, which note_is_debrief_drop.sql
drops.) `Save these` calls `applyNote` with what is still ticked. Walking
away instead dismisses the note, so nothing sits at `pending` forever.
