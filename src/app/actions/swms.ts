...
/** SORTED ON SITE. The answer to a raised issue is usually a new version of
    the SWMS, and sometimes it is a new anchor bolted in before the work
    starts. The person in charge says so, the issue stops asking, and the
    register keeps both: what was raised, and who sorted it. */
export async function clearSwmsIssue(personId: string): Promise<IssueResult> {
  const { orgId, userId } = await requireOrg();
  const viewer = await staffIdFor(orgId, userId);
  if (!viewer) return { ok: false, error: "Your account has no staff card." };
