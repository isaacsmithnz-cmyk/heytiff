...
const RATCHETS: Array<{ law: string; now: () => number; baseline: number }> = [
  ...
  { law: "gradients — one accent, flat surfaces", now: () => count(/(?:linear|radial|conic)-gradient\(/g), baseline: 97 },
  { law: "shadows that are not a focus ring — one shadow, overlays only", now: shadows, baseline: 213 },
  ...
  { law: "inner-highlight glass edges — no glass", now: () => count(/inset 0 1px 0 rgba\(255/g), baseline: 5 },
  ...
  { law: "letter-spacing — display titles only", now: () => count(/letter-spacing\s*:/g), baseline: 44 },
