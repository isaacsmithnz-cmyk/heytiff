/* Home reads its own address: `?task=<id>` is the bell's door onto a task
   ...
   with the id is pinned beside it (home-desk.test.tsx); this pins what the
   PAGE hands over — only a task id's shape, and nothing otherwise. */
...
async function homeFor(search: Search): Promise<Record<string, unknown>> {
  ...
  const home = tree.props.children.find((c) => c && c.type === DashboardDesk);
  return home!.props;
}
