  ...
  return (
    <div style={{ flex: "1 1 280px", minWidth: 0, background: "#fff", borderRadius: 18, border: `1px solid ${RC.line}`, boxShadow: "0 8px 30px rgba(0,0,0,.03)", padding: "20px 22px" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginTop: 10 }}>
        <div style={{ fontFamily: RC.head, fontWeight: 700, fontSize: 46, letterSpacing: "-0.03em", lineHeight: 0.9, color: RC.ink }}>
        ...
        {rec != null && cur != null && <div style={{ fontFamily: RC.head, fontWeight: 700, fontSize: 15, color: c, paddingBottom: 5, whiteSpace: "nowrap" }}>{Math.round(rec) - cur >= 0 ? "+" : ""}${Math.round(rec) - cur}/hr vs now</div>}
      </div>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
        {([["Break-even", rate0(be)], ["You charge", cur != null ? "$" + cur : "—"]] as [string, string][]).map(([l, v]) => (
          <div key={l} style={{ flex: 1, background: RC.card2, borderRadius: 11, padding: "9px 12px" }}>
            <div style={{ fontSize: 10, color: RC.faint, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em" }}>{l}</div>
            <div style={{ fontFamily: RC.head, fontWeight: 700, fontSize: 19, letterSpacing: "-0.01em", color: RC.ink2, marginTop: 2 }}>{v}</div>
          </div>
        ))}
      </div>
  ...
  return d && (
    <div style={{ flex: 1, minWidth: 0 }}>
      <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 9 }}>
        <span style={{ fontFamily: RC.head, fontWeight: 700, fontSize: 13.5, letterSpacing: "-0.01em", color: RC.ink }}>{label}</span>
        <span style={{ fontFamily: RC.head, fontWeight: 700, fontSize: 13.5, color: RC.ink2 }}>{rate0(d.rec)}<span style={{ color: RC.label, fontWeight: 700, fontSize: 11.5 }}>/hr</span></span>
      </div>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "6px 14px", marginTop: 10 }}>
        {d.segments.map(([l, v, c]) => v >= 0.5 && (
          <span key={l} style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 11.5, color: RC.ink2, whiteSpace: "nowrap" }}>
            <span style={{ width: 8, height: 8, borderRadius: 3, background: c, flexShrink: 0 }} />{l} <b style={{ fontFamily: RC.head, fontWeight: 700, color: RC.ink }}>${Math.round(v)}</b>
          </span>
        ))}
      </div>
    </div>
    ...
    return (
      <div className="rca-view" style={{ flex: 1, minWidth: 0, padding: "26px 32px", overflowY: "auto", overflowX: "hidden", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
        <div style={{ maxWidth: 460, textAlign: "center" }}>
          ...
          <div style={{ fontFamily: RC.head, fontWeight: 700, fontSize: 22, letterSpacing: "-0.02em", color: RC.ink, lineHeight: 1.15 }}>A little more to go</div>
    ...
    <div className="rca-view" style={{ flex: 1, minWidth: 0, padding: "18px 10px 6px 2px", overflowY: "auto", overflowX: "hidden", display: "flex", flexDirection: "column" }}>
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: 18 }}>
        <div>
          ...
          <div style={{ fontFamily: RC.head, fontWeight: 700, fontSize: 27, letterSpacing: "-0.02em", color: RC.ink, lineHeight: 1.1, marginTop: 8 }}>Your recommended rates</div>
        </div>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 14 }}>
        ...
        <div style={{ flex: "0 1 340px", minWidth: 300, background: RC.inkDeep, borderRadius: 18, padding: "20px 22px", color: "#fff", display: "flex", flexDirection: "column", position: "relative", overflow: "hidden" }}>
          ...
          <span style={{ display: "inline-flex", alignItems: "center", gap: 7, fontFamily: RC.head, fontSize: 10.5, fontWeight: 700, color: RC.teal, alignSelf: "flex-start", letterSpacing: "0.07em", textTransform: "uppercase", position: "relative" }}><span style={{ width: 7, height: 7, borderRadius: "50%", background: RC.teal }} />{applied ? "Rates applied" : alreadyHealthy ? "Charging a healthy rate" : "Ready to apply"}</span>
          <div style={{ fontFamily: RC.head, fontWeight: 700, fontSize: 20, letterSpacing: "-0.02em", lineHeight: 1.15, marginTop: 12, position: "relative" }}>{settled ? "You're priced right" : "You could recover"}</div>
          <div style={{ fontFamily: RC.head, fontWeight: 700, fontSize: 42, letterSpacing: "-0.03em", color: RC.teal, lineHeight: 1, marginTop: 6, position: "relative" }}>{settled ? "✓" : "+" + money(uplift)}</div>
          ...
          {alreadyHealthy ? (
            <div style={{ marginTop: "auto", position: "relative", background: "rgba(255,255,255,0.16)", color: "rgba(255,255,255,0.85)", borderRadius: 12, padding: "13px 0", fontFamily: RC.head, fontWeight: 700, fontSize: 14, textAlign: "center" }}>Keep your current rates ✓</div>
          ) : (
          <button onClick={apply} disabled={applied}
            style={{ marginTop: "auto", position: "relative", border: "none", background: applied ? "rgba(255,255,255,0.16)" : "#fff", color: applied ? "rgba(255,255,255,0.6)" : RC.inkDeep, borderRadius: 12, padding: "13px 0", fontFamily: RC.head, fontWeight: 700, fontSize: 14, cursor: applied ? "default" : "pointer", transition: "all .25s" }}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 14, marginTop: 14 }}>
        {([["After-hours", rate0(calc.afterHrs) + "/hr", RC.violet], ["Emergency", rate0(calc.emergency) + "/hr", RC.violet], ["Day rate, full day", money(calc.daily), RC.ink]] as [string, string, string][]).map(([l, v, c]) => (
          <div key={l} style={{ background: "#fff", borderRadius: 14, border: `1px solid ${RC.line}`, boxShadow: "0 8px 30px rgba(0,0,0,.03)", padding: "15px 17px" }}>
            <div style={{ fontSize: 10.5, color: RC.faint, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", whiteSpace: "nowrap" }}>{l}</div>
            <div style={{ fontFamily: RC.head, fontWeight: 700, fontSize: 23, letterSpacing: "-0.02em", color: c, marginTop: 5, whiteSpace: "nowrap" }}>{v}</div>
          </div>
        ))}
        <div style={{ background: "#fff", borderRadius: 14, border: `1px solid ${RC.line}`, boxShadow: "0 8px 30px rgba(0,0,0,.03)", padding: "15px 17px" }}>
          <div style={{ fontSize: 10.5, color: RC.faint, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", whiteSpace: "nowrap" }}>Utilisation</div>
          <div style={{ display: "flex", gap: 18, marginTop: 5 }}>
            {([["Install", Math.round(normUtil(calc.instUtil)), RC.install], ["Service", Math.round(normUtil(calc.svcUtil)), RC.service]] as [string, number, string][]).map(([l, v, c]) => (
              <div key={l}>
                <div style={{ fontFamily: RC.head, fontWeight: 700, fontSize: 23, letterSpacing: "-0.02em", color: c, whiteSpace: "nowrap" }}>{v}%</div>
