...
export const ICON_PATHS: Record<string, string> = {
  ...
  calc:
    '<rect width="16" height="20" x="4" y="2" rx="2"/><path d="M8 6h8"/><path d="M16 14v4"/><path d="M16 10h.01"/><path d="M12 10h.01"/><path d="M8 10h.01"/><path d="M12 14h.01"/><path d="M8 14h.01"/><path d="M12 18h.01"/><path d="M8 18h.01"/>',
  layers:
  ...
  /* Lucide "credit-card" — the company's own money, as against `receipt`,
     which is the docket for a purchase however it was paid. The pair reads on
     the expenses card: one door files what the card bought, the other asks for
     money back. */
  card: '<rect width="20" height="14" x="2" y="5" rx="2"/><line x1="2" x2="22" y1="10" y2="10"/>',
  box: '<path d="M21 8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16Z"/><path d="m3.3 7 8.7 5 8.7-5"/><path d="M12 22V12"/>',
  check: '<path d="M20 6 9 17l-5-5"/>',
  // Lucide "list-checks" — a ticked list, for the hero's task counter
  listCheck:
    '<path d="m3 7 2 2 4-4"/><path d="m3 17 2 2 4-4"/><path d="M13 6h8"/><path d="M13 12h8"/><path d="M13 18h8"/>',
  ...
  // Fault Finder symptom icons (Lucide: flame, power, droplet, snowflake, volume-2)
  flame:
  ...
  /* Staff-card glyphs. Copied from the injected renderer's private `EX` map so
     the React profile can use <Icon> without importing the HTML-string module
     (which is on its way out — see components/profile/). */
  phone:
  ...
  /* The capture pill's pair: a keyboard for "type it", a microphone for
     "say it". The mic is a real mic — `volume` is a speaker, which reads as
     playback, the opposite of what the button does. */
  keyboard:
    '<rect x="3" y="6" width="18" height="12" rx="2.5"/><path d="M7 10h.01M11 10h.01M15 10h.01M7 14h10"/>',
...
export function Icon({
  ...
  return <span dangerouslySetInnerHTML={{ __html: iconSvg(name, size, sw) }} />;
}
